...
export async function getProjectDashboard(projectId: string, projectTimezone: string | null): Promise<ProjectDashboardData> {
  ...
  const [programmingResult, guidesResult, ordersResult, batchResult, activityResult] = await Promise.all([
    ...
    supabase.from("batches").select("id, code, period_start, period_end, accounting_period, status, batch_dispatches(id, removed_at)").eq("project_id", projectId).lte("period_start", today).gte("period_end", today).limit(1).maybeSingle(),
  ...
  const current = batchResult.data as null | { id: string; code: string; period_start: string; period_end: string; accounting_period: string; status: BatchStatus; batch_dispatches: Array<{ removed_at: string | null }> | null };
  const currentBatch: DashboardBatch | null = current ? { id: current.id, code: current.code, periodStart: current.period_start, periodEnd: current.period_end, accountingPeriod: current.accounting_period, status: current.status, activeGuideCount: (current.batch_dispatches ?? []).filter((row) => row.removed_at === null).length } : null;
