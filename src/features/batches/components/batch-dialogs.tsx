...
export function Modal({ title, description, icon: Icon, onClose, pending, children, wide = false }: { title: string; description: string; icon: typeof Plus; onClose: () => void; pending: boolean; children: React.ReactNode; wide?: boolean }) {
  ...
  return <motion.div className="fixed inset-0 z-[85] grid place-items-center overflow-y-auto bg-black/50 p-3 backdrop-blur-[2px] sm:p-6" role="dialog" aria-modal="true" initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }}>
    <motion.div className={`w-full overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl ${wide ? "max-w-5xl" : "max-w-2xl"}`} initial={reduceMotion ? false : { y: 10 }} animate={{ y: 0 }}>
      <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4 sm:px-6"><div className="flex gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-brand-soft text-brand-strong"><Icon className="size-5" /></span><div><h2 className="font-semibold">{title}</h2><p className="mt-1 text-xs text-foreground-muted">{description}</p></div></div><button type="button" onClick={onClose} disabled={pending} className="grid size-9 place-items-center rounded-lg hover:bg-muted" aria-label="Cerrar"><X className="size-5" /></button></div>
      {children}
    </motion.div>
  </motion.div>;
}
...
export function AddDispatchDialog({ projectId, batchId, dispatches, onClose }: { projectId: string; batchId: string; dispatches: EligibleBatchDispatch[]; onClose: () => void }) {
  ...
  useGlobalPending(pending, "Agregando despacho…", "Validando que no pertenezca a otro lote activo.");
...
export function RemoveDispatchDialog({ projectId, batchId, relation, onClose }: { projectId: string; batchId: string; relation: BatchDispatchRelation; onClose: () => void }) {
  ...
  useGlobalPending(pending, "Removiendo despacho…", "Conservando la trazabilidad de la relación.");
  useEffect(() => { if (state.status === "success") router.refresh(); }, [router, state.status]);
...
export function RolloverDialog({ projectId, batchId, preview, onClose }: { projectId: string; batchId: string; preview: BatchRolloverPreview[]; onClose: () => void }) {
  ...
  useGlobalPending(pending, "Cerrando semana…", "Moviendo despachos pendientes y preservando el historial.");
  useEffect(() => { if (state.status === "success") router.refresh(); }, [router, state.status]);
...
function Footer({ onClose, pending, disabled, label, loading, icon }: { onClose: () => void; pending: boolean; disabled?: boolean; label: string; loading: string; icon?: boolean }) { return <div className="flex flex-col-reverse gap-3 border-t border-border px-5 py-4 sm:flex-row sm:justify-end sm:px-6"><button type="button" onClick={onClose} disabled={pending} className="secondary-button">Cancelar</button><LoadingButton disabled={disabled} loadingLabel={loading}>{icon && <CheckCircle2 className="size-4" />}{label}</LoadingButton></div>; }
