...
import type { GlobalReconciliationData, GlobalReconciliationItem } from "../types";
...
function tone(status: string): BadgeTone {
  ...
  if (status === "PENDING_RECONCILIATION") return "info";
  return "neutral";
}

function DocumentCompleteness({ item }: { item: GlobalReconciliationItem }) {
  return (
    <div className="space-y-1 text-xs">
      <p><span className="text-foreground-muted">Producto:</span> <strong>{item.hasProductInvoice ? "Cargado" : "Pendiente"}</strong></p>
      <p><span className="text-foreground-muted">Servicio:</span> <strong>{item.hasServiceInvoice ? "Cargado" : "Pendiente"}</strong></p>
    </div>
  );
}
...
export function ReconciliationWorkspace({ project, data }: { project: ProjectSummary; data: GlobalReconciliationData }) {
  ...
  const filtered = useMemo(() => data.items.filter((item) =>
    (!query || [item.orderNumber, item.programmingCode, item.supplierName].some((value) => value.toLowerCase().includes(query.toLowerCase()))) &&
    (!batch || item.batchId === batch) && (!status || item.reconciliationStatus === status),
  ), [data.items, query, batch, status]);
