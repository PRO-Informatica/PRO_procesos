...
export async function getOperationalNotifications(input: { projectId: string; projectName: string; userId: string; permissions: string[] }): Promise<OperationalNotification[]> {
  ...
  const [upcoming, overdue, orders, incidents, pendingDocs, reads] = await Promise.all([
    ...
    canInvoices ? supabase.from("reconciliation_orders").select("id, batch_id, normalized_order_number, reconciliation_status, updated_at").eq("project_id", input.projectId).in("reconciliation_status", ["NO_INVOICES", "WITH_DIFFERENCES", "PARTIAL", "REQUIRES_REVIEW"]).order("updated_at", { ascending: false }).limit(40) : Promise.resolve({ data: [], error: null }),
  ...
  for (const row of orders.data ?? []) {
    const difference = ["WITH_DIFFERENCES", "PARTIAL", "REQUIRES_REVIEW"].includes(row.reconciliation_status);
    push({ key: `order:${row.id}:${row.reconciliation_status}`, title: difference ? "Pedido requiere conciliación" : "Pedido pendiente de factura", description: difference ? `El Pedido ${row.normalized_order_number} tiene diferencias o necesita revisión.` : `El Pedido ${row.normalized_order_number} todavía no tiene factura PRODUCT.`, createdAt: row.updated_at, type: difference ? "RECONCILIATION" : "INVOICE", href: `/batches/${row.batch_id}/orders/${row.id}` });
  }
