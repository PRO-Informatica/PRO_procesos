...
export async function getOperationalNotifications(input: { projectId: string; projectName: string; userId: string; permissions: string[] }): Promise<OperationalNotification[]> {
  ...
  const [upcoming, overdue, orders, incidents, pendingDocs, reads] = await Promise.all([
    ...
    canInvoices ? supabase.from("dispatch_reconciliations").select("id, dispatch_id, status, updated_at").eq("project_id", input.projectId).in("status", ["NOT_STARTED", "WITH_DIFFERENCES", "PENDING_REINVOICING"]).order("updated_at", { ascending: false }).limit(40) : Promise.resolve({ data: [], error: null }),
