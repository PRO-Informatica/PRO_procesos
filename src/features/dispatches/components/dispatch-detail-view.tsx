...
function SectionHeader({ icon: Icon, title, count }: { icon: typeof Truck; title: string; count?: number }) {
  return <div className="flex items-start justify-between gap-3 border-b border-border px-4 py-3.5 sm:items-center sm:px-6 sm:py-4"><div className="flex min-w-0 items-start gap-2 sm:items-center"><Icon className="mt-0.5 size-5 shrink-0 text-brand-strong sm:mt-0" /><h2 className="break-words font-semibold">{title}</h2></div>{count !== undefined && <Badge>{count}</Badge>}</div>;
}
      ...
      <div className="flex flex-col gap-3 min-[420px]:flex-row min-[420px]:items-start min-[420px]:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 break-all font-semibold">{invoice?.number ?? "Pendiente"}</p>
          </div>
        </div>
        {invoice && <StatusBadge label={formatStatusLabel(invoice.status)} tone="success" />}
      </div>

      {invoice ? <>
        <p className="mt-5 break-words text-xl font-semibold tracking-tight text-foreground sm:text-2xl">
        ...
        {invoice.documentId && invoice.fileName && <div className="mt-5 flex flex-col gap-3 border-t border-border pt-4 min-[420px]:flex-row min-[420px]:items-center min-[420px]:justify-between"><p className="min-w-0 break-words text-xs text-foreground-muted [overflow-wrap:anywhere]" title={invoice.fileName}>{invoice.fileName}</p><div className="self-end min-[420px]:self-auto"><DocumentActions projectId={projectId} documentId={invoice.documentId} fileName={invoice.fileName} mimeType="application/pdf" getSignedUrl={getInvoiceDownloadUrl} /></div></div>}
  ...
  return (
    <li className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div className="min-w-0">
        ...
        <div className="mt-2"><StatusBadge label={formatStatusLabel(document.uploadStatus)} tone={document.uploadStatus === "UPLOADED" ? "success" : "neutral"} /></div>
      </div>
      <div className="flex justify-end gap-2">
        ...
        {editable && (
          <IconButton
            ...
            onClick={() =>
              startTransition(async () => {
                ...
                if (result.status === "success") {
                  notify.success(notifications.documentRemoved);
                  router.refresh();
                } else {
            ...
            tone="destructive"
            label={`Eliminar ${document.fileName ?? "evidencia"}`}
          >
...
export function DispatchDetailView({ detail, project, permissions }: { detail: DispatchDetail; project: ProjectSummary; permissions: DispatchPermissions }) {
  ...
  const [saveState, saveAction, saving] = useActionState(saveDispatchAction, initialDispatchMutationState);
  ...
  useActionNotification({ pending: saving, status: saveState.status, success: notifications.dispatchSaved, error: notifications.saveFailed });
  useActionNotification({ pending: completing, status: completeState.status, success: notifications.dispatchCompleted, error: notifications.actionFailed });
  useEffect(() => {
  ...
  return <MotionPage className="mx-auto w-full max-w-[1500px] space-y-4 pb-10 sm:space-y-6">
    <div><Link href="/dispatches" className="inline-flex min-h-11 items-center gap-2 text-sm font-semibold text-foreground-muted hover:text-foreground"><ArrowLeft className="size-4" /> Volver a despachos</Link><div className="mt-3 flex flex-wrap items-center gap-2 sm:mt-4 sm:gap-3"><div className="min-w-0 basis-full sm:basis-auto"><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Detalle de despacho</h1><p className="mt-1 break-words text-sm text-foreground-muted">{detail.programmingCode} · {detail.supplierName}</p></div><DispatchStatusBadge status={detail.status} /><DispatchResultBadge result={detail.result} /></div></div>

    <form action={saveAction} className="space-y-4 sm:space-y-6">
      ...
      <MotionSection className="overflow-hidden rounded-2xl border border-border bg-surface"><SectionHeader icon={Truck} title="Información general" /><div className="grid gap-4 p-4 min-[420px]:grid-cols-2 sm:gap-5 sm:p-6 lg:grid-cols-4"><div className="rounded-lg bg-muted/30 p-3 sm:bg-transparent sm:p-0"><p className="form-label">Programación</p><Link href={`/programming/${detail.programmingId}`} className="break-all font-mono text-sm font-semibold text-brand-strong hover:underline">{detail.programmingCode}</Link></div><div className="rounded-lg bg-muted/30 p-3 sm:bg-transparent sm:p-0"><p className="form-label">Proveedor</p><p className="break-words text-sm font-semibold">{detail.supplierName}</p></div><div className="rounded-lg bg-muted/30 p-3 sm:bg-transparent sm:p-0"><p className="form-label">Fecha</p><p className="text-sm font-semibold">{formatDispatchDateTime(detail.programmingScheduledAt, project.timezone)}</p></div><div className="rounded-lg bg-muted/30 p-3 sm:bg-transparent sm:p-0"><p className="form-label">Estado</p><DispatchStatusBadge status={detail.status} /></div><div className="min-[420px]:col-span-2 sm:col-span-1"><label className="form-label" htmlFor="dispatch-receiver">Receptor</label><input id="dispatch-receiver" name="receivedByName" value={receiverName} onChange={(event) => setReceiverName(event.target.value)} required disabled={!editable} className="form-input disabled:bg-muted" /></div><div><label className="form-label" htmlFor="dispatch-arrival">Hora llegada</label><input id="dispatch-arrival" type="time" value={arrivalTime} onChange={(event) => setArrivalTime(event.target.value)} required disabled={!editable} className="form-input disabled:bg-muted" /><p className="mt-1 text-xs text-foreground-muted">Día de la programación: {formatDispatchDate(operationDate)}</p></div><div><label className="form-label" htmlFor="dispatch-departure">Hora salida</label><input id="dispatch-departure" type="time" value={departureTime} onChange={(event) => setDepartureTime(event.target.value)} disabled={!editable} className="form-input disabled:bg-muted" /><p className="mt-1 text-xs text-foreground-muted">Final de la última entrega, el mismo día.</p></div><div className="min-[420px]:col-span-2 sm:col-span-1"><p className="form-label">Resultado</p><DispatchResultBadge result={detail.result} /></div></div></MotionSection>

      <MotionSection id="guides" className="overflow-hidden rounded-2xl border border-border bg-surface"><SectionHeader icon={PackageOpen} title="Guías de despacho" count={detail.guides.length} />{detail.guides.length ? <><div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-muted/50 text-xs text-foreground-muted"><tr><th className="px-5 py-3">Guía</th><th className="px-5 py-3">Fecha</th><th className="px-5 py-3">Productos</th><th className="px-5 py-3">Volumen</th><th className="px-5 py-3 text-right">Acciones</th></tr></thead><tbody className="divide-y divide-border">{detail.guides.map((item) => <tr key={item.id}><td className="px-5 py-4 font-semibold">{item.guideNumber}</td><td className="px-5 py-4 text-foreground-muted">{formatDispatchDate(item.guideDate)}</td><td className="px-5 py-4">{item.productCount}</td><td className="px-5 py-4 font-semibold">{formatDispatchQuantity(item.quantity)} {item.unitCode}</td><td className="px-5 py-4 text-right">{editable ? <button type="button" onClick={() => setGuide(item)} className="inline-flex min-h-11 items-center text-xs font-semibold text-brand-strong hover:underline">Ver / Editar</button> : <button type="button" onClick={() => setGuide(item)} className="inline-flex min-h-11 items-center text-xs font-semibold text-brand-strong hover:underline">Ver</button>}</td></tr>)}</tbody></table></div><div className="divide-y divide-border sm:hidden">{detail.guides.map((item) => <article key={item.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">Guía {item.guideNumber}</p><p className="mt-1 text-xs text-foreground-muted">{formatDispatchDate(item.guideDate)} · {item.productCount} producto(s)</p></div><p className="shrink-0 font-semibold">{formatDispatchQuantity(item.quantity)} {item.unitCode}</p></div><button type="button" onClick={() => setGuide(item)} className="secondary-button mt-3 w-full text-xs">{editable ? "Ver / Editar" : "Ver"}</button></article>)}</div><div className="flex flex-col gap-3 border-t border-border px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><p className="font-semibold">Total según guías: {formatDispatchQuantity(total)} {detail.programmedUnitCode}</p>{editable && <button type="button" onClick={() => setGuide("new")} className="primary-button w-full gap-2 sm:w-auto"><Plus className="size-4" /> Agregar guía</button>}</div></> : <div className="p-4 sm:p-6"><EmptyState title="Sin guías registradas" description={editable ? "Agrega la primera guía de esta operación." : "El despacho no tiene guías."} action={editable ? <button type="button" onClick={() => setGuide("new")} className="primary-button"><Plus className="size-4" /> Agregar guía</button> : undefined} /></div>}</MotionSection>
      ...
      {editable && <div className="sticky bottom-2 z-20 flex flex-col gap-2 rounded-xl border border-border bg-surface/95 p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] shadow-xl backdrop-blur sm:bottom-3 sm:flex-row sm:items-center sm:justify-end sm:gap-3 sm:rounded-2xl sm:p-4">{uploadedEvidenceCount === 0 && <p className="text-xs font-medium text-destructive sm:mr-auto">Carga al menos una evidencia antes de finalizar.</p>}<LoadingButton loadingLabel="Guardando…" className="w-full sm:w-auto">Guardar cambios</LoadingButton><Button type="button" variant="success" className="w-full sm:w-auto" disabled={uploadedEvidenceCount === 0 || saving} title={uploadedEvidenceCount === 0 ? "Carga al menos una evidencia antes de finalizar." : undefined} onClick={() => setConfirmOpen(true)}><CheckCircle2 className="size-4" /> Finalizar despacho</Button></div>}
    </form>
    ...
    {confirmOpen && completeState.status !== "success" && <Dialog title="¿Finalizar despacho?" description="Se guardarán los datos actuales y se cerrará la operación." icon={CheckCircle2} onClose={() => setConfirmOpen(false)} pending={completing} size="sm"><form action={completeAction}><div className="space-y-4 p-5"><input type="hidden" name="projectId" value={project.id} /><input type="hidden" name="dispatchId" value={detail.id} /><input type="hidden" name="programmingId" value={detail.programmingId} /><input type="hidden" name="expectedVersion" value={detail.version} /><input type="hidden" name="arrivalAt" value={arrivalTime ? `${operationDate}T${arrivalTime}` : ""} /><input type="hidden" name="departureAt" value={departureTime ? `${operationDate}T${departureTime}` : ""} /><input type="hidden" name="receivedByName" value={receiverName} /><input type="hidden" name="result" value={result} /><input type="hidden" name="orderNumber" value={orderNumber} /><input type="hidden" name="realVolume" value={result === "NOT_DISPATCHED" ? "0" : realVolume} /><input type="hidden" name="realUnitCode" value={realUnitCode} />{completeState.status === "error" && <p role="alert" className="rounded-lg bg-destructive-soft px-4 py-3 text-sm text-destructive">{completeState.message}</p>}</div><DialogFooter><Button variant="secondary" onClick={() => setConfirmOpen(false)} disabled={completing}>Cancelar</Button><LoadingButton variant="success" loadingLabel="Finalizando…">Finalizar despacho</LoadingButton></DialogFooter></form></Dialog>}
  </MotionPage>;
}
