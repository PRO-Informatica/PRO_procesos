...
export function DispatchDetailView({ detail, project, permissions }: { detail: DispatchDetail; project: ProjectSummary; permissions: DispatchPermissions }) {
  ...
  const [completeState, completeAction, completing] = useActionState(completeDispatchAction, initialDispatchMutationState);
  ...
  return <MotionPage className="mx-auto w-full max-w-[1500px] space-y-6 pb-10">
    ...
    <form action={saveAction} className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-border bg-surface"><SectionHeader icon={Truck} title="Información general" /><div className="grid gap-5 p-5 sm:grid-cols-2 lg:grid-cols-4 sm:p-6"><div><p className="form-label">Programación</p><Link href={`/programming/${detail.programmingId}`} className="font-mono text-sm font-semibold text-brand-strong hover:underline">{detail.programmingCode}</Link></div><div><p className="form-label">Proveedor</p><p className="text-sm font-semibold">{detail.supplierName}</p></div><div><p className="form-label">Fecha</p><p className="text-sm font-semibold">{formatDispatchDateTime(detail.programmingScheduledAt, project.timezone)}</p></div><div><p className="form-label">Estado</p><DispatchStatusBadge status={detail.status} /></div><div><label className="form-label" htmlFor="dispatch-receiver">Receptor</label><input id="dispatch-receiver" name="receivedByName" defaultValue={detail.receivedByName ?? ""} required disabled={!editable} className="form-input disabled:bg-muted" /></div><div><label className="form-label" htmlFor="dispatch-arrival">Hora llegada</label><input id="dispatch-arrival" name="arrivalAt" type="datetime-local" defaultValue={localInput(detail.arrivalAt, project.timezone)} required disabled={!editable} className="form-input disabled:bg-muted" /></div><div><label className="form-label" htmlFor="dispatch-departure">Hora salida</label><input id="dispatch-departure" name="departureAt" type="datetime-local" defaultValue={localInput(detail.departureAt, project.timezone)} disabled={!editable} className="form-input disabled:bg-muted" /><p className="mt-1 text-xs text-foreground-muted">Final de la última entrega.</p></div><div><p className="form-label">Resultado</p><DispatchResultBadge result={detail.result} /></div></div></section>
      ...
      {editable && <div className="sticky bottom-3 z-20 flex flex-col gap-3 rounded-2xl border border-border bg-surface/95 p-4 shadow-xl backdrop-blur sm:flex-row sm:items-center sm:justify-end"><LoadingButton loadingLabel="Guardando…">Guardar cambios</LoadingButton><button type="button" onClick={() => setConfirmOpen(true)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-foreground px-5 text-sm font-semibold text-surface"><CheckCircle2 className="size-4" /> Finalizar despacho</button></div>}
    </form>
