...
function SectionHeader({ icon: Icon, title, count }: { icon: typeof Truck; title: string; count?: number }) {
  return <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3.5 sm:px-6 sm:py-4"><div className="flex min-w-0 items-center gap-2"><Icon className="size-5 shrink-0 text-brand-strong" /><h2 className="truncate font-semibold">{title}</h2></div>{count !== undefined && <Badge>{count}</Badge>}</div>;
}
