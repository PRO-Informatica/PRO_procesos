...
export function RegisterDispatchDialog({
  ...
  const [programmingId, setProgrammingId] = useState(
    fixedProgrammingId ?? programming[0]?.id ?? "",
  );
  ...
  const [lines, setLines] = useState<Line[]>([
    {
      ...
      quantity: "",
      unitCode:
        programming.find((item) => item.id === fixedProgrammingId)?.unitCode ??
  ...
  useGlobalPending(
    ...
    "Creando guía, productos y trazabilidad operacional.",
  );
  useEffect(() => {
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="register-dispatch-title"
          initial={reduceMotion ? false : { opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={reduceMotion ? undefined : { opacity: 0 }}
        >
          <motion.div
            className="mx-auto my-auto w-full max-w-5xl overflow-hidden rounded-2xl border border-border bg-surface shadow-2xl"
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-4 sm:px-6">
              <div className="flex gap-3">
                ...
                <div>
                  <h2
                    id="register-dispatch-title"
                    className="font-semibold text-foreground"
                  >
                    Registrar despacho
                  </h2>
                  <p className="mt-1 text-xs text-foreground-muted">
                    Guía y recepción · zona horaria{" "}
                    {timezone || "America/Guatemala"}
                  </p>
                </div>
              </div>
            ...
            {state.status === "success" && state.dispatchId ? (
              <div className="grid place-items-center px-5 py-14 text-center sm:px-8">
                ...
                <p className="mt-2 max-w-md text-sm text-foreground-muted">
                  La guía, sus productos y las cantidades físicas quedaron
                  registrados como un solo expediente.
                </p>
                {canAttachDocument && state.guideId ? (
                  <div className="mt-6 w-full max-w-xl rounded-xl border border-border bg-muted/20 p-4 text-left sm:p-5">
                    ...
                    <p className="mt-1 text-xs leading-5 text-foreground-muted">
                      Adjunta ahora la foto, boleta o PDF físico de la guía. Si
                      la carga falla, el despacho permanece registrado y podrás
                      reintentar.
                    </p>
                    <div className="mt-4">
                      <DocumentUploader
                        projectId={projectId}
                        contextId={state.guideId}
                        context="guide"
                  ...
                  <p className="mt-4 max-w-md text-xs text-foreground-muted">
                    Puedes continuar al detalle para consultar el expediente y
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    type="button"
                    onClick={onClose}
                    className="inline-flex min-h-11 items-center justify-center rounded-lg border border-border px-4 text-sm font-semibold"
                  >
                    Cerrar
                  </button>
                  <Link
                    href={`/dispatches/${state.dispatchId}`}
                    className="primary-button"
                  >
                    Abrir detalle y continuar
                  </Link>
                </div>
              </div>
            ) : (
              <form action={action} aria-busy={pending}>
                ...
                <div className="grid gap-6 p-5 sm:p-6">
                  <section className="grid gap-4 rounded-xl border border-border bg-muted/20 p-4 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      <label
                        ...
                        htmlFor="dispatch-programming"
                      >
                        Programación
                      </label>
                      {fixedProgrammingId ? (
                        <div className="form-input bg-muted/40 font-mono font-semibold">
                          {formatIdentifier("PRG", programmingId)}
                        </div>
                      ) : (
                        <select
                          id="dispatch-programming"
                          required
                          className="form-input"
                          value={programmingId}
                          onChange={(event) => {
                            setProgrammingId(event.target.value);
                            const next = programming.find(
                    ...
                    <div>
                      ...
                      <div className="form-input bg-muted/40 font-medium">
                    ...
                    <div>
                      ...
                      <div className="form-input bg-muted/40 text-foreground-muted">
                        DISPATCH_GUIDE publicada · resolución automática
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                      <label className="form-label" htmlFor="guide-number">
                        Número de guía *
                      </label>
                      <input
                        id="guide-number"
                    ...
                    <div>
                      <label className="form-label" htmlFor="guide-date">
                        Fecha *
                      </label>
                      <input
                        id="guide-date"
                  ...
                  <section aria-labelledby="dispatch-products-title">
                    <div className="flex items-end justify-between gap-3">
                      <div>
                        <h3
                          id="dispatch-products-title"
                          className="text-sm font-semibold text-foreground"
                        >
                          Productos
                        </h3>
                        <p className="mt-1 text-xs text-foreground-muted">
                          Todas las líneas deben utilizar la misma UM.
                        </p>
                      </div>
                      <button
                        type="button"
                  ...
                  <section className="grid gap-4 sm:grid-cols-3">
                    <div>
                      <label className="form-label" htmlFor="load-time">
                        Hora de carga
                      </label>
                      <input
                        id="load-time"
                    ...
                    <div>
                      <label className="form-label" htmlFor="arrival-time">
                        Hora de llegada
                      </label>
                      <input
                        id="arrival-time"
                  ...
                  <section className="grid gap-4 rounded-xl border border-border p-4 lg:grid-cols-[minmax(13rem,0.8fr)_minmax(0,1.2fr)]">
                    <div>
                      <label className="form-label" htmlFor="dispatch-result">
                        Resultado físico
                      </label>
                      <select
                        id="dispatch-result"
                        ...
                        value={result}
                        onChange={(e) =>
                          setResult(e.target.value as DispatchResult)
                        }
                        className="form-input"
                      >
                      ...
                      <p className="mt-2 text-xs text-foreground-muted">
                        Independiente del estado del proceso, que inicia como
                        Registrado.
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                      ...
                      result === "REJECTED" ? (
                        <div>
                          <label className="form-label" htmlFor="sent-quantity">
                            Enviada
                          </label>
                          <input
                            id="sent-quantity"
                ...
                <div className="flex flex-col-reverse gap-3 border-t border-border px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
                  ...
                  <LoadingButton
                    loadingLabel="Registrando despacho…"
                    disabled={!programmingId}
                  >
                    Registrar despacho
                  </LoadingButton>
                </div>
